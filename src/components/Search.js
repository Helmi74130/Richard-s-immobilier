...
const SearchAll = () => {
  const [roomsValue, setRoomsValue]= useState('') 
    ...
    const handleButton = (e)=>{
      ...
      const fetchData =async () =>{
        await axios(`https://dd7548px.directus.app/items/product?fields=title,price,id,surface,type.name,city.name,price,thumbnail,rooms${valueProduct}${roomsValue}${valueCity}&filter[price][_between]=${valuePriceMin},${valuePriceMax}&filter[state][_eq]=vente&sort=-date_created`)
        .then( 
          response => {
    ...
    useEffect(()=>{
      const fetchData =async () =>{
        const result = await axios(`https://dd7548px.directus.app/items/product?fields=city.name,type.name&filter[state][_eq]=vente&sort=-date_created`)
  ...
  useEffect(()=>{
    const fetchData =async () =>{
      const result = await axios('https://dd7548px.directus.app/items/product?fields=title,price,id,surface,city.name,price,thumbnail,rooms,type.name&page=1&limit=4&filter[state][_eq]=vente&sort=-date_created')
  ...
  const testData =async (currentPage) =>{
    const result = await axios(`https://dd7548px.directus.app/items/product?fields=title,price,id,surface,city.name,price,thumbnail,rooms,type.name&page=${currentPage}&limit=4&filter[state][_eq]=vente&sort=-date_created`)
