...
const SearchAll = () => {
  const [roomsValue, setRoomsValue]= useState('') 
    ...
    const handleButton = (e)=>{
      ...
      const fetchData =async () =>{
        await axios(`http://localhost:8055/items/product?fields=title,price,id,surface,type.name,city.name,price,thumbnail,rooms${valueProduct}${roomsValue}${valueCity}&filter[price][_between]=${valuePriceMin},${valuePriceMax}&limit=4&filter[state][_eq]=location`)
        .then( 
          response => {
  ...
  useEffect(()=>{
    const fetchData =async () =>{
      const result = await axios('http://localhost:8055/items/product?fields=title,price,id,surface,city.name,price,thumbnail,rooms,type.name&page=1&limit=4&filter[state][_eq]=location')
  ...
  const testData =async (currentPage) =>{
    const result = await axios(`http://localhost:8055/items/product?fields=title,price,id,surface,city.name,price,thumbnail,rooms,type.name&page=${currentPage}&limit=4&filter[state][_eq]=location`)
